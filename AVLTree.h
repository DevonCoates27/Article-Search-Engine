...
public:
    ...
    Comparable* get(const Comparable &x, AvlNode *t) const {
        if (t == nullptr)
    ...
    //print given level
    void printGivenLevel(AvlNode* root, int level, ostream &out) const {
        if (root == nullptr)
            return;
        if (level == 1) {
            out << root->element << "\n";
    ...
    //non recursive level order output
    void levelOrderRecursive(ostream &out) const {
        printGivenLevelRecursive(root, height(root) + 1, out);
    }
    //print given level non recursive
    void printGivenLevelRecursive(AvlNode* root, int level, ostream &out) const {
        std::vector<AvlNode*> queue;
        queue.push_back(root);
        while(!queue.empty()) {
            AvlNode* node = queue[0];
...
private:
    //find function which returns the data of the node with the given key
